...
class Component extends WebComponent {
  ...
  constructor() {
    ...
    this.attr = attr => { // set as property because we want attr function to be this bound
      if (attr in this._attrsSchema) {
        const deprecatedMsg = this._attrsSchema[attr].deprecatedMsg;
        if (deprecatedMsg) {
          console.warn(`Panel: attr '${attr}' is deprecated. ${deprecatedMsg}`);
        }
        return this.attrs[attr];
      } else {
        throw new TypeError(`attr '${attr}' is not defined in attrSchema`);
      }
    };
   ...
   * Panel auto parses attribute changes into this.attrs object and $attr template helper
   *
